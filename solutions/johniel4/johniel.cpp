  ...
  while (true) {
    if (false && curr.boosters['B'] && curr.pend.size()) {
      ...
      if (m.inside(p.first, p.second) && !m.blocked(p.first, p.second)) {
        ...
        sprintf(buff, "B(%d,%d)", p.second, -p.first);
        t += string(buff);
        continue;
      }
    }
    ...
    if (mx.first <= 0) {
      ...
      each (c, s) {
        ...
        color();
        t += c;
        each (i, bs[make_pair(curr.i, curr.j)]) {
          ++curr.boosters[i];
        }
      }
    } else {
