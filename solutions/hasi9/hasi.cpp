...
void solve(const M& m, const char* filename) {
	static const int dy[] = {-1, 0,+1, 0};
	static const int dx[] = { 0,-1, 0,+1};
	...
	for (;;) {
		...
		if (state.cur_boosters['C'] > 0) {
			...
			sort(a.begin(), a.end());
			for (int j = 0; j < n && j < state.cur_boosters['C']; ++ j) {
				int d = a[j].first;
				int idx = a[j].second;
				used[idx] = true;
				if (d > 0) {
					...
					for (int i = 0; i < 4; ++ i) {
						...
						if (0 <= y && y < state.H && 0 <= x && x < state.W && spawn_dist[y][x] == d-1) {
							override.insert({idx, Command("WASD"[i], w.fast > 0)});
							break;
						}
					}
				}
