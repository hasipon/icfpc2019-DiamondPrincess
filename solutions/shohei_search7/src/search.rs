    ...
    'outer: for i in 0..repeat {
        ...
        loop {
...

            {
                ...
                current_score += result.get_score(position.width, position.height, &mut rng);

                if min_cost < history.len() {
                    continue 'outer;
                }
            }
            if rest_unwrapped <= 0 {
    ...
    for next_move in &moves {
        ...
        match position.apply(true, next_move) {
            Option::Some(result) => {
                ...
                score += result.get_score(position.width, position.height, rng) * (1.0 + rest as f64 * 0.1);
                if 0 < rest {
