...
struct State {
	...
	State(const M& m) {
		...
		workers.push_back(Worker(init_y, init_x));
	...
	void wrap(int idx) {
		...
		// cerr << "wrap" << idx << " " << w.y << " " << w.x << " " << w.mode << endl;
...
struct Solver {
	...
	int num_clone_used = 0;
	Command calc(int idx, int depth) {
		const auto& w = state.workers[sel_idx];
		if (state.get_boosters('C') - num_clone_used > 0 && state.spawn_points.count({w.y, w.x})) {
			++ num_clone_used;
			return Command('C');
		}
