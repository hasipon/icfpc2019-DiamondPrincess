...
struct State {
	...
	void input(int idx, const Command& cc) {
		...
		for (int t = 0; t < (cc.twice ? 2 : 1); ++ t) {
			...
			case 'C': workers.push_back(Worker(w.y, w.x)); if(cur_boosters['C'] <= 0){cerr<<"booster error C"<<endl;throw 1;} -- cur_boosters['C']; break;
...
struct Score {
	...
	void set_score_1(const Command& c, int depth, int rotten_size, int col_booster_count, int mani_size) {
		score1[c][-depth] = max(score1[c][-depth], rotten_size + col_booster_count * 100 + mani_size * 10000);
	}
...
struct Solver {
	...
	void wrap(set<pair<int,int>>& rotten2, int y, int x, int dir, const set<pair<int,int>>& mani1) {
		for (const auto& v : mani1) {
			...
			if (0 <= yy && yy < state.H && 0 <= xx && xx < state.W && state.rotten[yy][xx] && !rotten2.count({yy,xx})) {
				for (auto p : calc_obstacles(vv)) {
					if (!state.board[y + p.first][x + p.second]) goto next; // todo drill
				}
	...
	void walk(int depth, int y, int x, int dir, int fast, int drill, const set<pair<int,int>>& mani1, const set<pair<int,int>>& rotten1, const set<pair<int,int>>& visited1) {
		if (cmds.size() > 0) {
			if (mode == 1) {
				...
				score_obj.set_score_1(cmds[0], depth, rotten1.size(), col_booster_count, mani1.size());
			} else if (mode == 2) {
...
void solve(const M& m) {
	...
	for (;;) {
		...
		int n = inst.workers.size();
		for (int idx = 0; idx < n; ++ idx) {
			//cerr << "calc start" << endl;
			auto c = Solver(inst).calc(idx);
			//cerr << "calc end" << endl;
