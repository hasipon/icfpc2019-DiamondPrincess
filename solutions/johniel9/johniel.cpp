...
string nearest(const Worker& w, const M& m, const State& state)
{
  ...
  while (!q.empty()) {
    ...
    for (int d = 0; d < 4; ++d) {
      ...
      for (int len = 0; len < 1 + (bool)fast; ++len) {
        ni += di[d];
        nj += dj[d];
        f = false;
...
pair<int, string> hillClimbing(const string C, const M& m, State& state, const int limit)
{
  set<pair<int, int>> X;
  ...
  wrap(m, state);
  while (true) {
    if (limit < state.workers[0].cmd.size()) return {1 << 28, ""};
    bool f = true;
...
void solve(const M& m)
{
  ...
  do {
    if (++cnt % 6) continue;
    State state(m);
    auto p = hillClimbing(C, m, state, best.first);
