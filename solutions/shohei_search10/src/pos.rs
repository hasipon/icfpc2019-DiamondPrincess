...
impl Position {
    ...
    pub fn get_map_c(&self) -> HashSet<(i32, i32)> {
        let mut result = HashSet::new();
        for x in 0..self.width {
            for y in 0..self.height {
                if get_at(&self.map, x, y, self.width).state == CellState::C {
                    result.insert((x, y));
                }
            }
        }
    ...
    pub fn apply(&mut self, for_search:bool, mo:&Move) -> Option<MoveResult> {
        ...
        match mo {
            Move::W => {
                if self.is_out(x, y - 1, is_drill) { return Option::None; }
                self.get_body_mut().point.y -= 1;
                boost_item = self.get_boost_item();
                border_count += self.wrap(&mut wrapped);
            }
            Move::A => {
                if self.is_out(x - 1, y, is_drill) { return Option::None; }
    ...
    pub fn get_boost_item(&mut self) -> Option<BoostItem> {
        ...
        let result = match cell.state
        {
            ...
            CellState::Wall => { Option::Some(BoostItem::O) }
            _ => Option::None,
        };
        if result.is_some() {
    ...
    pub fn is_out(&self, x:i32, y:i32, is_drill:bool) -> bool {
        if is_drill {
            x < 0 || self.width <= x || y < 0 || self.height <= y
        } else {
    ...
    pub fn is_wrapped(&self, x:i32, y:i32) -> bool {
        self.is_out(x, y, false) || get_at(&self.map, x, y, self.width).is_wrapped
    }
    ...
    pub fn find_target(&self) -> Target {
        ...
        while let Option::Some(node) = nodes.pop_front() {
            ...
            let is_drill = (body.drill - node.moves.len() as i32) > 0;

            for (mo, dx, dy) in &ds
            {
                let mut dx = *dx;
                let mut dy = *dy;
                if is_fast && !self.is_out(x + dx, y + dy, is_drill) && !self.is_out(x + dx * 2, y + dy * 2, is_drill) {
                    dx *= 2;
                    dy *= 2;
                }
                if !self.is_out(x + dx, y + dy, is_drill) && !visited.contains(&(x + dx, y + dy)) { 
    ...
    pub fn get_available_moves(&self) -> Vec<Move> {
        ...
        if 0 < body.rest_l && body.drill <= 0
        {
            default_moves.push(Move::L);
        }
...
impl MoveResult {
    pub fn get_score(&self, position:&Position, width:i32, height:i32, rng:&mut ThreadRng) -> f64 {
        ...
        if 0.1 < score {
            ...
            score *= rng.gen_range(1.0, 1.12);
            let point = position.get_body().point;
            for body in &position.bodies {
                let d = (point.x - body.point.x).abs() + (point.y - body.point.y).abs();
                score += d as f64 * 0.20;
            }
        }
        score
