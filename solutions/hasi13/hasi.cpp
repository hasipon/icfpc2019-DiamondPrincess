...
struct Score {
	...
	void set_score_1(const Command& c, int depth, int rotten_size, int col_booster_count, int near_elder_count) {
		score1[c][-depth] = max(score1[c][-depth], rotten_size + col_booster_count * 100 + near_elder_count);
	}
	...
	void walk(int depth, int y, int x, int dir, int fast, int drill, const set<pair<int,int>>& mani1, const set<pair<int,int>>& rotten1, const set<pair<int,int>>& visited1) {
		if (cmds.size() > 0) {
			if (mode == 1) {
				...
				int near_elder_count = 0;
				for (int idx = 0; idx < sel_idx; ++ idx) {
					const auto& w = state.workers[idx];
					near_elder_count += max(0, 2 - (abs(w.x - x) + abs(w.y - y)));
				}
				score_obj.set_score_1(cmds[0], depth, rotten1.size(), col_booster_count, near_elder_count);
			} else if (mode == 2) {
