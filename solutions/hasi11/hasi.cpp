...
struct Score {
	map<Command, vector<int>> score1;
	vector<int> top1;
	map<Command, int> score2;
	int calc_score_1(int rotten_size, int col_booster_count) {
		return rotten_size + col_booster_count * 100;
	}
	void set_score_1(const Command& c, const vector<int>& a) {
		auto b = a;
		reverse(b.begin(), b.end());
		score1[c] = max(score1[c], b);
	}
	void set_score_2(const Command& c, int rotten_dist) {
		score2[c] = max(score2[c], (1<<30) - rotten_dist);
	}
	map<Command, vector<int>>::iterator get_top_1() {
		if (score1.empty()) {
...
struct Solver {
	...
	void walk(int depth, int y, int x, int dir, int fast, int drill, const set<pair<int,int>>& mani1, const set<pair<int,int>>& rotten1, const set<pair<int,int>>& visited1) {
		if (cmds.size() > 0) {
			if (mode == 1) {
				if (depth == 0) {
					score_obj.set_score_1(cmds[0], score_1);
				} else {
					...
					score_1.push_back(score_obj.calc_score_1(rotten1.size(), col_booster_count));
				}
			} else if (mode == 2) {
